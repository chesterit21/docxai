...
namespace Api.Services.Dms
{
    public class DocumentsService(IHttpContextAccessor accessor, ILanguageRepository languageRepository, IConfiguration configuration,
        ...
        IEmailRepository emailRepo, IUserRepository userRepo, CategoriesService categoryService, IAgentPollingTaskDocumentRepository agentPollingTaskRepo) : BaseService(accessor, languageRepository, configuration)

    {
        ...
        public async Task<Documents> Insert(RequestDocuments request)
        {
            ...
            await PopulateCategoryNamesAsync(entity);
        ...
        public async Task<Documents> UpdateDocument(RequestUpdateDocuments request)
        {
            ...
            await PopulateCategoryNamesAsync(exisitngDocument);
        ...
        private async Task PopulateCategoryNamesAsync(Documents entity)
        {
            var model = await categoryRepo.GetSingleAsync(x => x.Id == entity.CategoryID);
            if (model != null)
            {
                if (model.ParentId != null)
                {
                    entity.SubCategoryName = model.CategoryName;
                    var parentModel = await categoryRepo.GetSingleAsync(x => x.Id == model.ParentId.Value);
                    entity.CategoryName = parentModel?.CategoryName;
                }
                else
                {
                    entity.CategoryName = model.CategoryName;
                    entity.SubCategoryName = null;
                }
            }
        }
        ...
        public async Task<DocumentFiles> UploadNewVersionofFile(int documentId, IFormFile file, bool isMainDocumentFile)
        {
            ...
            if (isMainDocumentFile == true)
            {
                ...
                var doc = await repository.GetSingleAsync(x => x.Id == documentId);
                if (doc != null)
                {
                    doc.DocumentTypeName = fileExtension;
        ...
        public async Task<DocumentFiles> SetMainFileDocument(RequestDocumentFilesUpdateMainDocument dcf)
        {
            ...
            var doc = await repository.GetSingleAsync(x => x.Id == dcf.DocumentID);
            if (doc != null)
            {
                doc.DocumentTypeName = res.DocumentType;
                await repository.UpdateAsync(doc);
            }
        ...
        public async Task<int> InsertInitAddWithUploadDocument(RequestDocumentAddUpload request)
        {
            ...
            // 1. Ekstraksi Teks — branching berdasarkan tipe file
            string fullText = string.Empty;
            var mim = request.DocFile.ContentType;
            if (Mimes.Contains(mim))
            {
                // Image file: langsung OCR via Tesseract .NET SDK (in-memory, tanpa shell process)
                try
                {
                    fullText = FileProcessor.ProcessingOCR(filePath);
                }
                catch (Exception) { }
            }
            else
            {
                // Non-image file (PDF, DOCX, XLSX, dll): text extraction biasa
                try
                {
                    fullText = await FileProcessor.ExtractTextAsync(filePath);
                }
            ...
            AgentPollingTaskDocument pollingTask = new AgentPollingTaskDocument()
            {
                DocumentId = entity.Id,
            ...
            await agentPollingTaskRepo.InsertAsync(pollingTask);
        ...
        public async Task<int> UploadInitDocument(string categoryId, List<IFormFile> request)
        {
            ...
            // Query category SEKALI sebelum loop (menghilangkan N+1 query)
            var model = await categoryRepo.GetSingleAsync(x => x.Id == CatID);
            ...
            foreach (IFormFile file in request)
            {
                ...
                Documents dc = new Documents() { CategoryID = CatID };
                ...
                var filePath = DocumentFilesHelper.GetPhysicalPathForDocumentFile(UserId, UserName, model.Id, model.CategoryName, newFileName, true);
                var resourcePath = DocumentFilesHelper.GetResourcePathForDocumentFile(UserId, UserName, model.Id, model.CategoryName, newFileName);

                // Save file ke disk dulu sebelum buat FileInfo
                using (var stream = new FileStream(filePath, FileMode.Create))
                {
                    await file.CopyToAsync(stream);
                }

                // Ekstraksi Teks — branching berdasarkan tipe file
                string fullText = string.Empty;
                var mim = file.ContentType;
                if (Mimes.Contains(mim))
                {
