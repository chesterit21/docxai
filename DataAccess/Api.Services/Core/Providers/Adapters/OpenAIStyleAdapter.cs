...
/// </summary>
public class OpenAIStyleAdapter : OpenAIAdapter
{
    public override string ProviderName => "OpenAI-Style";
    ...
    public OpenAIStyleAdapter(
        HttpClient http,
        ProviderConfig config,
        IModelRegistry modelRegistry,
        ILogger<OpenAIStyleAdapter> logger)
        : base(http, config, modelRegistry, logger) { }

    protected override void ConfigureHttpClient()
    {
        _httpClient.BaseAddress = new Uri(_config.BaseUrl ?? "https://api.cohere.ai");
        _httpClient.DefaultRequestHeaders.Clear();
        _httpClient.DefaultRequestHeaders.Add("Authorization", $"Bearer {_config.ApiKey}");
        _httpClient.DefaultRequestHeaders.Add("Accept", "application/json");
    }

    protected override string DefaultEndpoint => "/compatibility/v1/chat/completions";
