...
namespace Api.Repository.Masters
{
    ...
    public class DocumentsRepository(DataContext context, IHttpContextAccessor accessor) : OwnerPrivilegesRepository<Documents>(context, accessor), IDocumentsRepository
    {
                ...
                try
                {
                    ...
                    if (request.DocumentFiles != null)
                    {
                        var model = await context.Categories
                            ...
                            .FirstOrDefaultAsync(x => x.Id == entity.CategoryID);
                        ...
                        if (model != null)
                        {
                            if (model.ParentId != null)
                            {
                                ...
                                entity.CategoryName = model.ParentCategory?.CategoryName;
                            }
                            else
                            {
                        ...
                        entity.DocumentTypeName = fileExtension;
        ...
        public async Task<ResponseSearchDocument> SearchListDocument(string SearchText, int page = 0, int limit = 0)
        {
            ...
            // Base query: access control filter
            ...
            // Count query: lightweight, tanpa Include/Select projection
            var totalCount = await baseQuery.CountAsync();

            // Pre-fetch favorites sebagai HashSet (batch query, bukan N+1)
            var favoriteDocIds = new HashSet<int>(
                await context.DocumentFavorites
                    .AsNoTracking()
            ...
            // Data query: dengan pagination dan projection
            ...
            if (page > 0 && limit > 0)
                dataQuery = dataQuery.Skip(skip).Take(limit);

            var rawResults = await dataQuery.ToListAsync();

            var finalResults = rawResults.Select(x => new DocumentList()
            {
                CategoryID = x.Doc.CategoryID,
                CategoryName = x.Doc.CategoryName,
                DocumentId = x.Doc.Id,
                DocumentName = x.Doc.DocumentTitle,
                Description = x.Doc.DocumentDesc,
                Owner = x.Doc.Owner,
                OwnerName = x.OwnerName,
                FilesSize = SizeFormatter.SizeSuffix((long)(x.Doc.FileSize ?? 0), 2),
                InsertedAt = x.Doc.InsertedAt,
                IsFavorite = favoriteDocIds.Contains(x.Doc.Id),
                FileType = x.Doc.DocumentTypeName,
        ...
        public async Task<ResponseSearchDocument> AdvSearchDocument(string DocumentTitle, DateTime? DtFrom, DateTime? DtTo, int? CategoryId, int page = 0, int limit = 0)
        {
            ...
            {
                baseQuery = baseQuery.Where(x => x.UpdatedAt.Date >= DtFrom.Value.Date && x.UpdatedAt.Date <= DtTo.Value.Date);
            }
            ...
            // Data query: pagination + projection
            var documents = await baseQuery
                ...
                .Select(x => new DocumentList
                {
                    ...
                    DocumentId = x.Id,
                    DocumentName = x.DocumentTitle,
                    Description = x.DocumentDesc,
                    Owner = x.Owner,
                    OwnerName = x.OwnerInfo.UserName,
                    LastUpdateDate = x.UpdatedAt,
                    FilesSize = SizeFormatter.SizeSuffix((long)(x.FileSize ?? 0), 2),
                    InsertedAt = x.InsertedAt,
                    InsertedBy = x.InsertedBy,
                    UpdatedAt = x.UpdatedAt,
                    UpdatedBy = x.UpdatedBy,
                    FileType = x.DocumentTypeName,
                    InsertedByUserName = x.InsertedByUserName,
                    InsertedByFullName = x.InsertedByFullName,
                    UpdatedByUserName = x.UpdatedByUserName,
                    UpdatedByFullName = x.UpdatedByFullName,
                    IsFavorite = favoriteDocIds.Contains(x.Id)
                })
                .ToListAsync();
