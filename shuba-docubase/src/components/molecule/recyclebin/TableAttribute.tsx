...
const TableList = () => {
  ...
  const getDataTable = () => {
    ...
    apiClient
      .get(
        `/attribute/recyclebin?search=${search}&Page=${page}&Limit=${pageLength}`
      )
      .then(({ data }) => {
  ...
  const handleDeleteOk = () => {
    if (eventData) {
      ...
      apiClient
        .delete(`/attribute/hard-delete?id=${eventData}`)
        .then(({ data }) => {
  ...
  const handleEmptyOk = () => {
    ...
    apiClient
      .delete(`/attribute/empty-recyclebin`)
      .then(({ data }) => {
      ...
      >
        <div style={{ padding: "10px 24px" }}>
          <p className="font-bold text-lg" style={{ marginBottom: 0 }}>
            Are you sure you want to delete permanent an Attribute?
          </p>
