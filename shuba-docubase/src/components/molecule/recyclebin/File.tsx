...
const TableList = () => {
  ...
  const getDataTable = () => {
    ...
    apiClient
      .get(
        `/documentfiles/recyclebin?search=${search}&Page=${page}&Limit=${pageLength}`
      )
      .then(({ data }) => {
  ...
  const handleClickRestore = async (value: any) => {
    ...
    await apiClient
      .put(`/documentfiles/undelete?id=${id}`)
      .then(({ data }) => {
        console.log(data);
        getDataTable();
  ...
  const handleDeleteOk = () => {
    if (eventData) {
      ...
      apiClient
        .delete(`/documentfiles/hard-delete?id=${eventData}`)
        .then(({ data }) => {
  ...
  const handleEmptyOk = () => {
    ...
    apiClient
      .delete(`/documentfiles/empty-recyclebin`)
      .then(({ data }) => {
