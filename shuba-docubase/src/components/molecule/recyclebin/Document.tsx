...
const TableList = () => {
  ...
  const getDataTable = () => {
    ...
    apiClient
      .get(
        `/documents/recyclebin?search=${search}&Page=${page}&Limit=${pageLength}`
      )
      .then(({ data }) => {
  ...
  const handleDeleteOk = () => {
    if (eventData) {
      ...
      apiClient
        .delete(`/documents/hard-delete?id=${eventData}`)
        .then(({ data }) => {
  ...
  const handleEmptyOk = () => {
    ...
    apiClient
      .delete(`/documents/empty-recyclebin`)
      .then(({ data }) => {
