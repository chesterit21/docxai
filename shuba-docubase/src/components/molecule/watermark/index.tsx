...
import ModalAddEditWatermark from "./ModalAddEditWatermark";
import ModalViewWatermark from "./ModalViewWatermark";
...
const WatermarkList = () => {
  ...
  const getData = () => {
    ...
    apiClient
      .get(`/watermarks?page=${page}&limit=${pageLength}${searchParam}`)
      .then(({ data }) => {
        setData(data?.data?.records || []);
        setTotalRow(data?.data?.totalRecord || 0);
        setLoading(false);
      })
      .catch((err) => {
        console.log(err);
  ...
  const handleOkDelete = () => {
    if (!eventData || !eventData.id)
      return message.error("Watermark ID not found!");
    setConfirmDeleteLoading(true);
    apiClient
      .put(`/watermarks/delete?id=${eventData.id}`)
      .then(({ data }) => {
        console.log(data);
        ...
        setEventData({});
        getData();
        message.success("Delete watermark is successful");
      })
      .catch((err) => {
        console.log(err);
        message.error(err.response?.data?.message || "Delete watermark failed");
        setConfirmDeleteLoading(false);
        setOpenConfirmDelete(false);
      });
  };
  ...
  return (
    <>
      <Breadcrumb
        item={[
          { title: <span style={{ fontWeight: "bold" }}>Watermark</span> },
        ]}
      />
      ...
      >
        <div style={{ padding: "10px 24px" }}>
          ...
          <span className="text-gray-500">
            Deleting this watermark will permanently remove it. Make sure you
