...
export default function Index({open, setOpen}: any) {
    ...
    const handleOkAdd = async () => {
        try {
            ...
            formData.append('request', fileList[0]);
            
            setConfirmLoading(true);
            await apiClient.post(`/user/upload-profile`, formData,
            {
                headers: {
    ...
    const props: UploadProps = {
        ...
        beforeUpload: (file) => {
            ...
            if (!isLt1MB) {
                message.error('File size cannot be more than 1 MB!');
                return Upload.LIST_IGNORE;
            }
            setFileList([file]);
            return false;
        ...
        <>
            ...
            <div className="space-align-block" style={{paddingLeft: 24, paddingRight: 24, paddingTop: 24}}>
                ...
                <Space.Compact direction="vertical">
                    ...
                    <Text type="secondary" style={{fontWeight: 'normal'}}>Upload your avatar.</Text>
                </Space.Compact>
                </Space>
            </div>
            )}
            ...
            >
                <div style={{padding: '10px 24px'}}>
                    <p className="font-bold text-lg" style={{marginBottom: 0}}>Are you sure you want to add an avatar?</p>
                    <span className="text-gray-500">The avatar you uploaded will be saved as your avatar.</span>
                </div>
            </Modal>
        </>
    )
