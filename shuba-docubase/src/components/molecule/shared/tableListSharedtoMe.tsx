...
const TableList = () => {
    ...
    const getDataDocument = ()=> {
        ...
        apiClient.get(`/sharedworkspace/sharedtome?DocumentOrCategoryTitle=${searchDocument}&Page=${pageDocument}&Limit=${pageLengthDocument}`)
        .then(({ data }) => {
    ...
    return (
        <>
            ...
            <Row>
                <Col span={24}>
                    <TableComplexShared type='shared_to_me' data={dataDocument} setData={setDataDocument} pageSize={pageLengthDocument} page={pageDocument} setPage={setPageDocument} totalRow={totalRowDocument} loading={loadingDocument} rerenderData={getDataDocument}/>
                </Col>
            </Row>
        </>
    )
