...
export default function Index({ open, setOpen, document, rerenderData }: any) {
  ...
  const fetchDataWhenOpen = async () => {
    ...
    try {
      ...
      const { data: docRes } = await apiClient.get(
        `/documents/get-document-dtl?id=${document}&mode=share`
      );
      let sharedUsers: any = docRes?.data?.sharedTo?.sharedUser || [];
