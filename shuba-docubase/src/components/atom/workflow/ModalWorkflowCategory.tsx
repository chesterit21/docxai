...
export default function Index({ open, setOpen, category, rerenderData }: any) {
  ...
  const fetchDataWhenOpen = async () => {
    setLoading(true);
    await apiClient
      .get(`/category/get-workflow?categoryId=${category.id}`)
      .then(({ data }) => {
        setUserExist(data?.data?.approvalFlows || []);
    ...
    try {
      ...
      if (userExist.length > 0) {
        filter = filter.filter(
          (a: any) => !userExist.some((b: any) => b.user.userID === a.userID)
        );
      }
      setUserList(filter);
    } catch (err) {
  ...
  const handleConfrim = async () => {
    ...
    const approval = { categoryID: category.id, ...formFields };
    const dataStore = { approval: approval, flows: joinUser };
    setConfirmLoading(true);
    await apiClient
      .post("/category/add-workflow", dataStore)
      .then(({ data }) => {
        console.log(data);
  ...
  //         "userID": item.userID
  //     }
  //     await apiClient.post('/documents/delete-workflow-user', dataStore)
  //     .then(({ data }) => {
  //         console.log(data)
  ...
  const handleOkDeleteWorkflow = () => {
    ...
    apiClient
      .delete(`category/delete-workflow?categoryId=${category.id}`)
      .then(({ data }) => {
        console.log(data);
  ...
  return (
    <>
                    ...
                    <List.Item>
                      <div className="flex flex-row justify-between items-center w-full">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          {/* <Button type="text" icon={<Trash2 size={18} color='#595959'/>} iconPosition="start" onClick={()=> handleDeletUserExist(item.user, item.approvalID)} /> */}
                        </div>
                      </div>
                    </List.Item>
                  )}
      ...
      >
        <div style={{ padding: "10px 24px" }}>
          <p className="font-bold text-lg" style={{ marginBottom: 0 }}>
            Are you sure you want to delete workflow this Category?
          </p>
          <span className="text-gray-500">
